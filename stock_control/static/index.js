...
const ConfirmDialog = {
  ...
  show(options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      // handleEsc se define antes de handleAccept/handleCancel para que
      // ambos puedan removerlo al resolver el diálogo.
      const handleEsc = (e) => {
        if (e.key === 'Escape') {
          handleCancel();
        }
      ...
      // Foco en Cancelar para evitar confirmación accidental con Enter/Space.
...
if (codigoScanner) {
  codigoScanner.addEventListener("keydown", async (event) => {
    if (!modalAbierta && !modoDevolucion) {
      Toast.warning("Abrí una operación (Ingresar, Retirar o Devolución) antes de escanear.");
      return;
    }
    if (event.key === "Enter") {
      event.preventDefault();
...
async function abrirModal(tipo) {
  ...
  // Arrancar siempre en paso 1
  _wizardIrPaso(tipo, 1);
...
function cerrarModal(tipo) {
  ...
  setTimeout(() => {
    ...
    modal.classList.remove("fade-out");
    modalContent?.classList.remove("zoom-out");
    _wizardIrPaso(tipo, 1);  // resetear UI del wizard al paso 1 para la próxima apertura
    desactivarInputEscaneo(); // después de _wizardIrPaso para no re-habilitarlo
  }, 300);
}

/* ─── Helpers de skeleton ───────────────────────────────────────────────── */
/** Espera dos frames para que el navegador pinte el DOM antes de continuar */
function _awaitPaint() {
  return new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
}

/** Fade-out de todos los .skeleton de un contenedor, luego resuelve */
async function _fadeSkeletons(container) {
  const items = container?.querySelectorAll('.skeleton');
  if (!items?.length) return;
  items.forEach(s => s.classList.add('skeleton-fadeout'));
  await new Promise(r => setTimeout(r, 280));
}

/* ─── Helpers de animación unificados ───────────────────────────────────── */
function _openModalGenerico(modal) {
  if (!modal) return;
...
function _closeModalGenerico(modal, onClosed) {
  if (!modal) return;
  const content = modal.querySelector(".modal-content");
...
function actualizarListaEscaneados(modalTipo, lista) {
  ...
  // Contador para numerar unidades del mismo código (ingreso/retiro)
  const _contadorCodigo = {};
  ...
  for (const producto of listaCompleta) {
    ...
    if (modalTipo === "devolucion") {
      ...
      // Ingreso / retiro: mostrar "Unidad N" cuando hay múltiples del mismo código
      const totalMismoCodigo = listaCompleta.filter(p => p.codigo_barras === producto.codigo_barras).length;
      _contadorCodigo[producto.codigo_barras] = (_contadorCodigo[producto.codigo_barras] ?? 0) + 1;
      const numUnidad = _contadorCodigo[producto.codigo_barras];
      const prefijo = totalMismoCodigo > 1 ? `Unidad ${numUnidad} · ` : "";

      li.textContent = `🧊 ${prefijo}${producto.nombre} · ${producto.peso} kg · ${producto.codigo_barras}`;

      const btn = document.createElement("button");
      btn.classList.add("btnEliminar");
      btn.textContent = "✕";
      btn.addEventListener("click", () =>
        eliminarProductoEscaneado(producto.codigo_barras, modalTipo, producto.scan_item_id)
      );
      li.appendChild(btn);
    }
...
async function eliminarProductoEscaneado(codigoBarras, modalTipo, scanItemId) {
  try {
    // Usar scan_item_id cuando está disponible para eliminar el balde físico exacto
    // (evita borrar la unidad equivocada cuando hay múltiples con el mismo código)
    const payload = scanItemId
      ? { scan_item_id: scanItemId }
      : { codigo_barras: codigoBarras };
    const data = await postJSON(API.eliminarTemporal, payload);
    if (data?.success) {
...
async function actualizarTablaStock() {
  ...
  if (tablaStock) {
    const skelRows = Array.from({ length: 4 }, () =>
      `<tr><td><div class="skeleton skeleton-table-row" style="width:70%;height:14px;border-radius:4px;"></div></td>` +
      `<td><div class="skeleton skeleton-table-row" style="width:40%;height:14px;border-radius:4px;margin:0 auto;"></div></td></tr>`
    ).join("");
    tablaStock.innerHTML = `<thead><tr><th>Producto</th><th>Cantidad de Baldes</th></tr></thead><tbody>${skelRows}</tbody>`;
  }

  // Garantizar que el navegador pinte el skeleton antes de hacer el fetch
  await _awaitPaint();
  ...
  try {
    ...
    // Fade-out del skeleton antes de reemplazar
    await _fadeSkeletons(tablaStock);
    ...
    body += "</tbody>";
    if (tablaStock) tablaStock.innerHTML = encabezado + body;
...
async function procesarCodigoEscaneado(codigo) {
  try {
    ...
    // Código ya en la lista: pedir confirmación antes de agregar otra unidad
    if (data?.duplicado) {
      const agotado = modo === "retirar" && data.ya_en_lista >= data.stock_disponible;
      if (agotado) {
        Toast.error(
          `No hay más baldes de ${data.nombre} disponibles ` +
          `(${data.stock_disponible} en stock, ${data.ya_en_lista} ya en la lista).`
        );
        return;
      }
      const confirmar = await ConfirmDialog.show({
        title: "Código ya escaneado",
        message: "Este código ya fue escaneado. ¿Se trata de otro balde físico con el mismo código?",
        icon: "📦",
        type: "info",
        acceptText: "Agregar otra unidad",
        cancelText: "Cancelar: es el mismo balde",
      });
      if (!confirmar) return;

      // Forzar el agregado de la unidad adicional
      const data2 = await postJSON(API.procesarCodigo, { codigo, force_duplicate: true });
      if (data2?.error) { Toast.error(data2.error); return; }
      Toast.success("Segunda unidad agregada", null, 1500);
      navigator.vibrate?.(50);
      await obtenerProductosEscaneados();
      if (modo === "retirar") await validarStockParaRetiro();
      return;
    }
...
async function confirmarAgregarProductos() {
  ...
  try {
    ...
    if (data._status === 409) {
      if (data.se_puede_forzar) {
        const fecha = data.fecha_ingreso
          ? new Date(data.fecha_ingreso).toLocaleString("es-AR")
          : "-";

        const texto = `
          ${data.mensaje}
          Producto: <b>${data.producto ?? "-"}</b>
          Peso previo: <b>${data.peso_anterior ?? "-"}</b>
      ...
      Toast.error(data.mensaje || "Conflicto detectado");
      return;
    }
...
function mostrarModalDuplicado(mensaje, onConfirm) {
  ...
  newAceptar.addEventListener("click", () => {
    _closeModalGenerico(modal, () => { if (typeof onConfirm === "function") onConfirm(); });
  });

  newCancelar.addEventListener("click", () => {
    _closeModalGenerico(modal);
  });
}
 ...
 * 11) CRUD PRODUCTOS                       *
 *******************************************/
let _modoProducto = "crear"; // "crear" | "editar"
let _pluEditando   = null;
...
function abrirModalCrearProducto() {
  _modoProducto = "crear";
  _pluEditando  = null;
  byId("modal-producto-form-titulo").textContent = "Crear Producto";
  byId("prod-plu").value    = "";
  byId("prod-plu").disabled = false;
  byId("prod-nombre").value = "";
  byId("prod-minimo").value = "";
  const pluRow = byId("prod-plu-row");
  if (pluRow) pluRow.style.display = "";
  _openModalGenerico(byId("modal-producto-form"));
  setTimeout(() => byId("prod-plu").focus(), 50);
}
...
function editarProducto(plu, nombreActual, minimoActual) {
  _modoProducto = "editar";
  _pluEditando  = plu;
  byId("modal-producto-form-titulo").textContent = "Editar Producto";
  byId("prod-plu").value    = plu;
  byId("prod-plu").disabled = true;
  byId("prod-nombre").value = nombreActual || "";
  byId("prod-minimo").value = minimoActual ?? "";
  const pluRow = byId("prod-plu-row");
  if (pluRow) pluRow.style.display = "";
  _openModalGenerico(byId("modal-producto-form"));
  setTimeout(() => byId("prod-nombre").focus(), 50);
}

function cerrarModalProductoForm() {
  _closeModalGenerico(byId("modal-producto-form"));
}

async function guardarProductoForm() {
  const nombre = (byId("prod-nombre")?.value || "").trim();
  if (!nombre) { Toast.warning("Ingresá un nombre para el producto"); return; }

  if (_modoProducto === "crear") {
    const plu    = (byId("prod-plu")?.value    || "").trim();
    const minimo = (byId("prod-minimo")?.value || "").trim();
    if (!plu) { Toast.warning("Ingresá el PLU"); return; }
    try {
      const data = await postJSON("/api/crear_producto/", { nombre, plu, stock_minimo: minimo });
      if (data.success) {
        Toast.success("Producto creado correctamente");
        cerrarModalProductoForm();
        abrirAdminProductos();
      } else {
    ...
    const minimo = (byId("prod-minimo")?.value || "").trim();
    try {
      const data = await postJSON("/api/actualizar_producto/", {
        plu:          _pluEditando,
        nombre,
        stock_minimo: minimo
      });
...
let _stockDetalleVisible = false;

function toggleStockDetalle() {
  _stockDetalleVisible = !_stockDetalleVisible;
  const container = byId("stock-detalle");
  const btn       = byId("btn-toggle-stock");
  if (container) container.style.display = _stockDetalleVisible ? "block" : "none";
...
async function checkForUpdate() {
  try {
    ...
    if (data.error) return; // sin conexión o servidor sin soporte — ignorar silenciosamente
...
async function abrirModalDevolucion() {
  ...
  try {
    ...
    console.error("⚠️ No se pudo reiniciar la lista de temporales:", err);
    Toast.warning("No se pudo limpiar la lista anterior. Revisá la conexión.");
  }
  ...
  _openModalGenerico(byId("modal-devolucion"));
  activarInputEscaneo();
}
...
function cerrarModalDevolucion() {
  ...
  pesosEditados = {};
  _devWizardIrPaso(1);      // resetear UI del wizard antes de desactivar scanner
  desactivarInputEscaneo();
  if (codigoScanner) codigoScanner.style.visibility = "hidden";
  _closeModalGenerico(byId("modal-devolucion"));
}
...
/*
 * ─── Wizard genérico para ingresar / retirar (2 pasos) ───
 *
 * Paso 1: Escanear baldes
 * Paso 2: Seleccionar origen (ingresar) / boca de salida (retirar)
 */

/** Navega al paso indicado (1 o 2) del wizard de ingresar/retirar. */
function _wizardIrPaso(tipo, paso) {
  const prefix = tipo === "ingresar" ? "ing" : "ret";
  [1, 2].forEach(n => {
    const stepEl = byId(`${prefix}-step-${n}`);
    const dotEl  = byId(`${prefix}-step-dot-${n}`);
...
/** Paso 1 → 2: valida y avanza. */
async function wizardSiguiente(tipo) {
  if (!productosEscaneados || productosEscaneados.length === 0) {
    Toast.warning("Escaneá al menos un balde antes de continuar");
    return;
  }
  if (tipo === "retirar") {
    const ok = await validarStockParaRetiro();
  ...
  _wizardIrPaso(tipo, 2);
}

/** Paso 2 → 1: vuelve al escaneo. */
function wizardVolver(tipo) {
  _wizardIrPaso(tipo, 1);
  setTimeout(() => codigoScanner?.focus(), 100);
}
...
async function procesarCodigoDevolucion(codigo) {
  try {
    ...
    // BUG-FIX: si el código ya estaba en la lista, data.duplicado=true y
    // data.productos_temporales no existe → asignar [] vaciaba la lista visual.
    // Las devoluciones no soportan dos unidades del mismo código (el backend
    // deduplica silenciosamente), así que bloqueamos la segunda unidad aquí.
    if (data?.duplicado) {
      Toast.warning(
        `El código de ${data.nombre} ya fue escaneado en esta devolución. ` +
        `Cada código puede devolverse una sola vez por operación.`
      );
      return; // No tocar productosEscaneados ni la lista visual
    }
...
async function confirmarDevolucion() {
  ...
  try {
    const _tempData = await getJSON(API.obtenerTemporales);
    const productosEscaneados = _tempData.productos || [];
...
async function abrirModalCrearBoca(tipo) {
  ...
  _openModalGenerico(byId("modalCrearBoca"));
...
async function cargarDashboard() {
    ...
    try {
        const url = currentCategoria
            ...
            : CONFIG.API_URL;
        dashboardData = await getJSON(url);
...
function aplicarPeriodoCustom() {
    ...
    if (!fechaDesde || !fechaHasta) {
        Toast.warning('Por favor seleccioná ambas fechas');
        return;
    }

    // Validar que desde sea menor que hasta
    if (new Date(fechaDesde) > new Date(fechaHasta)) {
        Toast.warning('La fecha "Desde" debe ser anterior a la fecha "Hasta"');
        return;
    }
...
async function cargarDashboardCustom(desde, hasta) {
    ...
    try {
        ...
        Toast.error('Error al cargar los datos del período seleccionado');
    } finally {
        mostrarLoading(false);
    }
}
